...
//------------ST7735_Message------------
// String draw and number output. Prints a string and signed integer,
...
void ST7735_Message(uint32_t d, uint32_t l, char *pt, int32_t value)
{
    ...
    OS_bWait(&LCDFree);
    ST7735_SetCursor(0, 8 * d + l);
    ST7735_DrawString(0, StY, "                     ", StTextColor); // clear line
    ST7735_OutString(pt);

    // Add space between string and number
    StX++;
