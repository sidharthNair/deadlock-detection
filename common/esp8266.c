...
#include "../common/WifiSettings.h" // access point parameters
...
bool ReceiveDataFilter(char letter)
{
    ...
    case 4: // filter out data and put it into the right receive FIFO
        if (ReceiveDataLen)
        {
            ...
            {
            case 0:
                if (ESP8266Rx0Fifo_Put(letter) == FIFOFAIL)
                { // overflow, data loss
...
{
    UART_ESP8266(_OutChar)
    (data);
    if (ESP8266_EchoCommand)
        UART_OutChar(data); // echo debugging
}
...
int ESP8266_Init(int rx_echo, int tx_echo)
{
    ...
    while (*s)
    {
        ...
        if (rx_echo)
            UART_OutCharNonBlock(c); // echo, requires UART0 to be operational, will print garbage
        if (c == *s)
        {
            s++;
        }
...
int ESP8266_Receive(char *fetch, uint32_t max)
{
    ...
    while (max > 1)
    {
        if (ESP8266Rx0Fifo_Size() || ESP8266_DataAvailable)
        { // data (about to be) available?
        ...
        { // wait for next packet or connection close
            if (ESP8266_ConnectionMux)
            {
                s = "0,CLOSED";
            }
