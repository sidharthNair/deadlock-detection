...
uint32_t PeriodicJitterHist1[JITTERSIZE]={0,};
int32_t PeriodicJitter2;
uint32_t PeriodicJitterHist2[JITTERSIZE]={0,};
...
#define OSCRITICAL_EXIT() { dt = OS_TimeDifference(t1, OS_Time()); SumCritical += dt; if (dt > MaxCritical) { MaxCritical = dt; } EndCritical(sr); }
#else
...
int OS_AddThread(void(*task)(void),
  ...
  stack_pool[tid][STACK_SIZE-11] = (tcb_pool[tid].process != NULL) ?
    (uint32_t)(tcb_pool[tid].process->data) : 0x09090909;    // R9
...
int OS_ProcessAddInitialThread(void(*task)(void),
  ...
  stack_pool[tid][STACK_SIZE-9] = 0x11111111;     // R11
  stack_pool[tid][STACK_SIZE-10] = 0x10101010;    // R10
  stack_pool[tid][STACK_SIZE-11] = (process != NULL) ?
    (uint32_t)(process->data) : 0x09090909;    // R9
  stack_pool[tid][STACK_SIZE-12] = 0x08080808;    // R8
  stack_pool[tid][STACK_SIZE-13] = 0x07070707;    // R7
  stack_pool[tid][STACK_SIZE-14] = 0x06060606;    // R6
  stack_pool[tid][STACK_SIZE-15] = 0x05050505;    // R5
