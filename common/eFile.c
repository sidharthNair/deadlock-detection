...
#define ACQUIRE_SEMA() { OS_bWait(&LCDFree); }
#define RELEASE_SEMA_AND_RETURN(x) { OS_bSignal(&LCDFree); return x; }
...
int eFile_Init(void){ // initialize file system
  ...
  if(initstatus) RELEASE_SEMA_AND_RETURN(1);

  int result = eDisk_Init(0);
  if (result) RELEASE_SEMA_AND_RETURN(result);
...
int eFile_Create( const char name[]){  // create new file, make it empty
  ...
  if (dir.size == 29) RELEASE_SEMA_AND_RETURN(1);

  for (uint8_t i = 0; i < dir.size; i++) {
    if (dir.entries[i].type == IS_FILE) {
      if (strcmp(dir.entries[i].name, name) == 0) { // don't allow duplicate names
...
// Output: 0 if successful and 1 on failure (e.g., trouble writing to flash)
int eFile_DirCreate( const char name[]){  // create new directory, make it empty
...
int eFile_WOpen( const char name[]){      // open a file for writing
  ...
  for (i = 0; i < dir.size; i++){
    if (dir.entries[i].type == IS_FILE){
      if (strcmp(dir.entries[i].name, name) == 0){ // compare name
        break;
      }
    }
  }
...
int eFile_WClose(void){ // close the file for writing
  ...
  for (i = 0; i < dir.size; i++){
    if (dir.entries[i].type == IS_FILE){
      if (dir.entries[i].start == write_file.start_block){ // find file
        break;
      }
    }
  }
...
int eFile_DOpen( const char name[]){ // open directory
  ...
  if (dir_open) RELEASE_SEMA_AND_RETURN(1);
  ...
  if (strcmp(name, "") == 0) {
    ...
    for (i = 0; i < opened_dir.size; i++){
      if (opened_dir.entries[i].type == IS_DIR){
        if (strcmp(opened_dir.entries[i].name, name) == 0){ // compare name
          break;
        }
      }
    }
