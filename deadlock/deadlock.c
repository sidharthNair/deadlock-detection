...
void heapStats(void){
  stats = Heap_Stats();
  ST7735_Message(1,0,"blocksUnused  =",stats.blocksUnused);
  ST7735_Message(1,1,"blocksUsed  =",stats.blocksUsed);
  ST7735_Message(1,2,"wordsAllocated  =",stats.wordsAllocated);
  ST7735_Message(1,3,"wordsAvailable =",stats.wordsAvailable);
  ST7735_Message(1,4,"wordsOverhead =",stats.wordsOverhead);
}
...
void TestHeap(void){  int16_t i;
  ...
  *ptr = 0x1111;
  ...
  for(i = 0; i <= stats.wordsAvailable; i++){
    ptr = Heap_Malloc(sizeof(int16_t));
    if(!ptr) break;
  }
  ...
  *q1 = 0xDD;
  ...
  heapStats();
  maxBlockSize = stats.wordsAvailable * 4;
...
void TestProcess(void){ heap_stats_t heap1, heap2;
  ...
  heap1 = Heap_Stats();
  PD1 ^= 0x02;
  ST7735_Message(1,0,"blocksUnused  =",heap1.blocksUnused);
  ST7735_Message(1,1,"blocksUsed  =",heap1.blocksUsed);
  ST7735_Message(1,2,"wordsAllocated  =",heap1.wordsAllocated);
  ST7735_Message(1,3,"wordsAvailable =",heap1.wordsAvailable);
  ST7735_Message(1,4,"wordsOverhead =",heap1.wordsOverhead);
  PD1 ^= 0x02;
  ...
  heap2 = Heap_Stats();
  PD1 ^= 0x02;
  ST7735_Message(1,0,"blocksUnused  =",heap2.blocksUnused);
  ST7735_Message(1,1,"blocksUsed  =",heap2.blocksUsed);
  ST7735_Message(1,2,"wordsAllocated  =",heap2.wordsAllocated);
  ST7735_Message(1,3,"wordsAvailable =",heap2.wordsAvailable);
  ST7735_Message(1,4,"wordsOverhead =",heap2.wordsOverhead);
  PD1 ^= 0x02;
  if((heap1.wordsAvailable != heap2.wordsAvailable)||(heap1.wordsAvailable != heap2.wordsAvailable)){
