...
int AP_SendNotification(uint32_t i)
{
    ...
    int r1;
    uint32_t s;
    if (i >= NotifyCharacteristicCount)
        return APFAIL; // not valid
    if (NotifyCharacteristicList[i].CCCDvalue)
    { // send only if active
        handle = NotifyCharacteristicList[i].theHandle;
        if (handle == 0)
            return APFAIL;                                                        // not open
...
void AP_BackgroundProcess(void)
{
    ...
    if (AP_RecvStatus())
    {
        if (AP_RecvMessage(RecvBuf, RECVSIZE) == APOK)
        {
            ...
            if ((RecvBuf[3] == 0x55) && (RecvBuf[4] == 0x88))
            {                                       // SNP Characteristic Write Indication (0x88)
                ...
                while (i < MAXCHARACTERISTICS)
                {
                    if (CharacteristicList[i].theHandle == h)
                    {
                        ...
                        if (count > s)
                            count = s; // truncate to size
                        d = s - count;
                        for (j = 0; j < s; j++)
                        {                                    // if message is smaller than size
            ...
            if ((RecvBuf[3] == 0x55) && (RecvBuf[4] == 0x87))
            ...
            if ((RecvBuf[3] == 0x55) && (RecvBuf[4] == 0x8B))
            {                                       // SNP CCCD Updated Indication (0x8B)
                h = (RecvBuf[8] << 8) + RecvBuf[7]; // handle for this characteristic
                responseNeeded = RecvBuf[9];
                for (i = 0; i < NOTIFYMAXCHARACTERISTICS; i++)
                {
                    if (NotifyCharacteristicList[i].CCCDhandle == h)
                    { // to do
